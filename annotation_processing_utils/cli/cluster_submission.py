                    ...
                    for iteration in range(
                        ...
                        if len(failed_info)==0 or (run, roi_name, iteration) in failed_info:
                            ...
                            for current_postprocessing_sweep in postprocessing_sweep:
                                current_log_path = Path(f"{log_path}_{dict_to_suffix(current_postprocessing_sweep)}")
                                ...
                                if submission_type == "mws":
                                    ...
                                    if "adjacent_edge_bias" in current_postprocessing_sweep:
                                        args["--adjacent_edge_bias"] = current_postprocessing_sweep["adjacent_edge_bias"]
                                    if "lr_biases" in current_postprocessing_sweep:
                                        args["--lr_biases"] = " ".join(str(v) for v in current_postprocessing_sweep["lr_biases"])
                                    if "filter_val" in current_postprocessing_sweep:
                                        args["--filter_val"] = current_postprocessing_sweep["filter_val"]
                    ...
                    if len(parts) == 2:
                        ...
                        if ': ' in reason_command:
                            ...
                            try:
                                ...
                                print(f"Warning: Could not parse job number from: {job_num}")
...
def get_failed_jobs_from_array_dir(job_array_dir):
    ...
    if not commands_file.exists():
        raise ValueError(f"Commands file {commands_file} not found in job array directory")
    
    if not failures_log.exists():
        print(f"Warning: No failures.log found in {job_array_dir}. No failed jobs to resubmit.")
        return []
    ...
    if script_file.exists():
        with open(script_file, 'r') as f:
            ...
            for line in lines:
                line = line.strip()
                if (line.startswith('#BSUB') and 
                    not line.startswith('#BSUB -J') and 
                    not line.startswith('#BSUB -o') and 
                    not line.startswith('#BSUB -e')):
    ...
    for job_idx in sorted(failed_jobs.keys()):
        if 1 <= job_idx <= len(commands):
            ...
            if log_path:
                full_command = f"bsub {bsub_opts_str} -o {log_path}.o -e {log_path}.e {command}"
            else:
...
def generic_submitter(submission_type):
    ...
    if args.resubmit_failures and args.resubmit_from_array:
        print("Error: Cannot specify both --resubmit-failures and --resubmit-from-array")
        return
    ...
    if args.resubmit_failures:
        ...
        try:
            arguments_per_submission = get_failed_jobs_from_array_dir(args.resubmit_from_array)
            if not arguments_per_submission:
                print("No failed jobs found in the job array directory")
                return
            print(f"Found {len(arguments_per_submission)} failed jobs to resubmit from {args.resubmit_from_array}")
...
def submit_job_array_with_retries(arguments_per_submission, submission_type, submission_info_path, 
                                auto_resubmit=False, max_retries=3):
    ...
    while retry_count <= max_retries and remaining_jobs:
        if retry_count == 0:
            ...
            print(f"Retry {retry_count}/{max_retries}: {len(remaining_jobs)} failed jobs")
        
        # Submit the current batch of jobs
        job_array_dir = submit_job_array(remaining_jobs, submission_type, submission_info_path)
        ...
        try:
            ...
            if retry_count >= max_retries:
                print(f"Maximum retries ({max_retries}) reached. {len(failed_commands)} jobs still failing.")
    ...
    if retry_count > max_retries:
        print(f"Final result: {len(remaining_jobs)} jobs failed after {max_retries} retries")
...
def submit_job_array(arguments_per_submission, submission_type, submission_info_path):
    ...
    job_array_dir = Path(f"/nrs/cellmap/{username}/job_arrays/{yaml_name}_{submission_type}_{timestamp}")
                ...
                for i in range(1, num_jobs + 1):
                    ...
                    if job_log.exists():
                        try:
                            ...
                            if "Successfully completed" in content or "Exited" in content:
                                completed_count += 1
                        except:
                            # If we can't read the file, skip it
                            pass
                
                print(f"Job {job_id} status: {status} | Progress: {completed_count}/{num_jobs} jobs completed ({completed_count/num_jobs*100:.1f}%)")
    ...
    for i in range(1, num_jobs + 1):
        ...
        if job_log.exists():
            ...
            if "Exited" in content:
                ...
                failures.append(f"Job {i} failed - no completion status found: {command}")
                with open(status_dir / f"job_{i}.status", 'w') as f:
