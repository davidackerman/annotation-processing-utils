...
class ZarrWriteSelectChannels(BatchFilter):
    ...
    Args:
        ...
        channel_slices (``slice``):

            Slice of channels to write out
    """
    ...
    def process(self, batch, request):
        ...
        with self._open_file(self.store) as data_file:
            for array_key, dataset_name in self.dataset_names.items():
                ...
                channel_slices = (
                    (self.channel_slices,)
                    if self.channel_slices
                    else (slice(None),) * max(0, len(dataset.shape) - dims)
                )
